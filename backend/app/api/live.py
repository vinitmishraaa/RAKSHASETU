...
async def fetch_firms(client: httpx.AsyncClient, key: str, source: str, bbox: tuple[float, float, float, float]):
    ...
    for row in csv.DictReader(io.StringIO(response.text)):
        ...
        records.append({
            ...
            "type": "Fire Hotspot",
            "title": "NASA FIRMS near-real-time active fire detection",
            "lat": lat,
            "lng": lng,
            "frp": frp,
            ...
            "detail": f"FRP {frp:.1f} MW · confidence {row.get('confidence') or 'n/a'} · satellite {row.get('satellite') or source}",
...
async def live_hazards(region: str | None = Query(default=None)):
    ...
    bbox = REGION_BBOXES.get(region or "India", REGION_BBOXES["India"])
    items = []
    sources = []
    async with httpx.AsyncClient(timeout=15, headers={"User-Agent": "RakshaSetu-Demo/1.0"}) as client:
        try:
            ...
            for feature in r.json().get("features", []):
                ...
                severity = "CRITICAL" if mag >= 6 else "HIGH" if mag >= 4.5 else "MODERATE" if mag >= 3 else "LOW"
                items.append({
                    "id": feature.get("id"), "type": "Earthquake", "title": props.get("title"),
                    "lat": float(coords[1]), "lng": float(coords[0]), "magnitude": mag,
                    "time": props.get("time"), "severity": severity, "source": "USGS",
                    "detail": f"Magnitude {mag:.1f} · depth {float(coords[2] or 0):.1f} km",
                    "url": props.get("url"),
                })
            sources.append({"name": "USGS Earthquake Feed", "status": "live", "count": len([x for x in items if x["type"] == "Earthquake"])})
        except Exception as exc:
            sources.append({"name": "USGS Earthquake Feed", "status": f"unavailable: {str(exc)[:100]}"})

        if settings.FIRMS_API_KEY:
            fire_count = 0
            for sensor in ("VIIRS_NOAA20_NRT", "VIIRS_NOAA21_NRT"):
                try:
                    records = await fetch_firms(client, settings.FIRMS_API_KEY, sensor, bbox)
                    items.extend(records[:800]); fire_count += len(records)
                except Exception as exc:
                    sources.append({"name": f"NASA FIRMS · {sensor}", "status": f"unavailable: {str(exc)[:100]}"})
            sources.append({"name": "NASA FIRMS", "status": "live", "count": fire_count})
        else:
            sources.append({"name": "NASA FIRMS", "status": "key_required"})
    ...
    return {
        ...
        "note": "Live public observations. Satellite fire detections and earthquake feeds are signals, not official evacuation orders; verify official warnings before action.",
    }
