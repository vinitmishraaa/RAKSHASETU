...
def relocation_plan(village_id: str):
    ...
    return {"village_id": village["id"], "village_name": village["name"], "population": plan["population"], "best_site": plan["best_site"], "ranked_sites": ranked, "allocations": plan["allocations"], "fully_covered": plan["fully_covered"], "reason": plan["reason"]}

@router.get("/route/{village_id}/{site_id}")
async def relocation_route(village_id: str, site_id: str):
    village = synthetic.get_village(village_id)
...
def all_plans():
    ...
    for village in synthetic.get_villages():
        plan = build_relocation_plan(village, sites)
        plans.append({"village_id": village["id"], "village_name": village["name"], "population": plan["population"], "best_site": plan["best_site"], "allocations": plan["allocations"], "fully_covered": plan["fully_covered"], "reason": plan["reason"]})
    return plans
