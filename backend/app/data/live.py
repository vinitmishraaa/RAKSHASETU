...
def _risk_from_observations(precip_mm: float | None, wind_kmh: float | None, earthquake_distance_km: float | None) -> float:
    ...
    quake_component = 0.0 if earthquake_distance_km is None else max(0.0, 100.0 - earthquake_distance_km * 4.0)
...
async def get_live_settlements(region: str | None = None) -> list[dict]:
    ...
    query = f"""[out:json][timeout:30];(node[place~\"^(village|town|city)$\"]({south},{west},{north},{east}););out tags center;"""
    data = await _overpass(query)
    items = []
    for element in data.get("elements", []):
        tags = element.get("tags", {}); lat = element.get("lat"); lon = element.get("lon")
        if lat is None or lon is None: continue
        population = None
        try:
            if tags.get("population"): population = int(float(str(tags["population"]).replace(",", "")))
        except (TypeError, ValueError): pass
        items.append({"id": f"osm-{element['type']}-{element['id']}", "name": tags.get("name") or "Unnamed mapped settlement", "district": tags.get("addr:district"), "state": tags.get("addr:state") or region, "region": region or "India", "lat": float(lat), "lng": float(lon), "population": population, "population_source": "OpenStreetMap population tag" if population is not None else None, "source": "OpenStreetMap", "source_url": f"https://www.openstreetmap.org/{element['type']}/{element['id']}"})
    return items


async def get_live_shelters(region: str | None = None) -> list[dict]:
    south, north, west, east = bbox_for(region)
    query = f"""[out:json][timeout:30];(node[amenity=shelter]({south},{west},{north},{east});way[amenity=shelter]({south},{west},{north},{east});relation[amenity=shelter]({south},{west},{north},{east});node[emergency=shelter]({south},{west},{north},{east});way[emergency=shelter]({south},{west},{north},{east});relation[emergency=shelter]({south},{west},{north},{east}););out center tags;"""
    data = await _overpass(query)
    items = []
    for element in data.get("elements", []):
        tags = element.get("tags", {}); center = element.get("center", {}); lat = element.get("lat", center.get("lat")); lon = element.get("lon", center.get("lon"))
        if lat is None or lon is None: continue
        items.append({"id": f"osm-shelter-{element['type']}-{element['id']}", "name": tags.get("name") or "Mapped emergency shelter", "region": region or "India", "lat": float(lat), "lng": float(lon), "capacity": None, "current_occupancy": None, "available_capacity": None, "hazard_risk": None, "infrastructure_score": None, "facilities": [v for v in [tags.get("amenity"), tags.get("emergency"), tags.get("access")] if v], "verified": False, "source": "OpenStreetMap", "source_url": f"https://www.openstreetmap.org/{element['type']}/{element['id']}", "note": "Mapped location only. Capacity, occupancy and operational status are not inferred."})
    return items
    ...
    async with httpx.AsyncClient(timeout=20, headers={"User-Agent": "RakshaSetu/1.0"}) as client:
        for item in settlements:
            ...
            try:
                ...
                pass
            precipitation = weather.get("precipitation"); wind = weather.get("wind_speed_10m")
