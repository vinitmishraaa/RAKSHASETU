...
def _area_query(kind: str, region: str | None):
    ...
    for name in targets:
        ...
        blocks.append(f'area["ISO3166-2"="IN-{code}"][boundary=administrative][admin_level=4]->.{code};')
        if kind == "node":
            nodes.append(f'nwr(area.{code})[place~"^(village|town|city)$"];')
        else:
            nodes.extend([
                f"node(area.{code})[amenity=shelter];", f"way(area.{code})[amenity=shelter];",
                f"relation(area.{code})[amenity=shelter];", f"node(area.{code})[emergency=shelter];",
                f"way(area.{code})[emergency=shelter];", f"relation(area.{code})[emergency=shelter];",
            ])
    return "[out:json][timeout:60];(" + "".join(blocks) + "(" + "".join(nodes) + "););out center tags;", targets
...
async def _district_relation(state: str, district: str):
    ...
    if not code:
        return None
    query = (
        f'[out:json][timeout:60];area["ISO3166-2"="IN-{code}"][boundary=administrative][admin_level=4]->.state;'
        f'relation(area.state)[boundary=administrative][admin_level=6]["name"="{district.replace(chr(34), chr(39))}"];out tags center;'
    )
    ...
    if not elements:
        query = (
            f'[out:json][timeout:60];area["ISO3166-2"="IN-{code}"][boundary=administrative][admin_level=4]->.state;'
            f'relation(area.state)[boundary=administrative][admin_level=5]["name"="{district.replace(chr(34), chr(39))}"];out tags center;'
        )
        data = await _overpass(query)
        elements = data.get("elements", [])
...
async def get_live_settlements(region=None, district=None):
    if district and region in REGION_CODES:
        ...
        if relation:
            ...
            query = f'[out:json][timeout:60];nwr(area:{area_id})[place~"^(village|town|city)$"];out center tags;'
            data = await _overpass(query)
            targets = [region]
        else:
            data = {"elements": []}
    ...
    for element in data.get("elements", []):
        ...
        try:
            ...
            population = None
        state = region if region in REGION_CODES else tags.get("addr:state")
        items.append({
            "id": f"osm-{element['type']}-{element['id']}",
            "name": tags.get("name:en") or tags.get("name") or "Unnamed mapped settlement",
            "district": district or tags.get("addr:district"),
            "state": state,
            "region": state or region or "India",
            "lat": float(lat), "lng": float(lon), "population": population,
...
async def get_live_shelters(region=None, district=None):
    if district and region in REGION_CODES:
        ...
        if relation:
            area_id = int(relation["id"]) + 3600000000
            query = (
                f'[out:json][timeout:60];(node(area:{area_id})[amenity=shelter];way(area:{area_id})[amenity=shelter];'
                f'relation(area:{area_id})[amenity=shelter];node(area:{area_id})[emergency=shelter];'
                f'way(area:{area_id})[emergency=shelter];relation(area:{area_id})[emergency=shelter];);out center tags;'
            )
            data = await _overpass(query)
        else:
            data = {"elements": []}
    else:
        query, _ = _area_query("shelter", region)
        data = await _overpass(query)
    ...
    for element in data.get("elements", []):
        tags = element.get("tags", {})
        center = element.get("center") or {}
        lat = element.get("lat", center.get("lat")); lon = element.get("lon", center.get("lon"))
        if lat is None or lon is None:
            continue
        items.append({
            "id": f"osm-shelter-{element['type']}-{element['id']}",
            "name": tags.get("name:en") or tags.get("name") or "Mapped emergency shelter",
            "region": region or "India", "district": district,
            "lat": float(lat), "lng": float(lon), "capacity": None,
            "current_occupancy": None, "available_capacity": None, "hazard_risk": None,
...
async def _earthquakes():
    async with httpx.AsyncClient(timeout=20, headers={"User-Agent": "RakshaSetu/1.0"}) as client:
        response = await client.get(USGS_URL); response.raise_for_status()
    ...
    for feature in features:
        ...
        out.append({"lat": float(coords[1]), "lng": float(coords[0]), "magnitude": float(props.get("mag") or 0),
                    "time": props.get("time"), "place": props.get("place") or "Earthquake", "id": feature.get("id")})
    return out
...
def _distance_km(lat1, lon1, lat2, lon2):
    x = radians(lon2 - lon1) * cos(radians((lat1 + lat2) / 2)); y = radians(lat2 - lat1)
        ...
        for start in range(0, len(settlements), 80):
            ...
            try:
                response = await client.get(OPEN_METEO_URL, params={
                    "latitude": ",".join(str(x["lat"]) for x in batch),
                    "longitude": ",".join(str(x["lng"]) for x in batch),
                    "current": "temperature_2m,precipitation,wind_speed_10m", "timezone": "UTC",
                })
                weather_rows = response.json().get("current", [])
                if isinstance(weather_rows, dict): weather_rows = [weather_rows]
            except Exception:
                weather_rows = []
            for index, item in enumerate(batch):
                weather = weather_rows[index] if index < len(weather_rows) else {}
                nearest = min(((_distance_km(item["lat"], item["lng"], q["lat"], q["lng"]), q) for q in earthquakes),
                              default=(None, None), key=lambda x: x[0] if x[0] is not None else 1e12)
                distance, quake = nearest
                score = _risk_from_observations(weather.get("precipitation"), weather.get("wind_speed_10m"), distance)
                item.update({
                    "weather": {"temperature_c": weather.get("temperature_2m"), "precipitation_mm": weather.get("precipitation"),
                                "wind_speed_kmh": weather.get("wind_speed_10m")},
                    "nearest_earthquake_km": round(distance, 1) if distance is not None else None,
                    "nearest_earthquake": quake, "risk_score": score, "level": _level(score),
...
async def _gdacs_events(region=None):
    ...
    for item in root.findall(".//item"):
        title = _clean(item.findtext("title")) or "GDACS event"
        description = _clean(item.findtext("description")); link = _clean(item.findtext("link"))
        guid = _clean(item.findtext("guid")) or link or title; pub_date = _clean(item.findtext("pubDate"))
        lat = lon = None
        ...
        items.append({"id": f"gdacs-{guid}", "type": "GDACS Alert", "title": title, "lat": lat, "lng": lon,
                      "time": pub_date, "severity": "ALERT", "source": "GDACS", "detail": description[:500],
