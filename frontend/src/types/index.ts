...
export interface Village {
  ...
  risk_score: number; risk_components?: Record<string, number>; hazard?: number | null; exposure?: number | null; vulnerability?: number | null; accessibility?: number | null;
  ...
  nearest_earthquake_km?: number | null; nearest_earthquake?: { magnitude?: number | null; place?: string; time?: number | null } | null; risk_model?: string; observed_at?: string; data_status?: string;
}
...
export interface SafeSite {
  id: string; name: string; region?: string; district?: string | null; lat: number; lng: number;
...
export interface SiteSuitability { site_id: string; site_name: string; suitability?: number; distance_km: number; available_capacity?: number | null; road_access?: "GOOD" | "MODERATE" | "POOR" | "UNVERIFIED"; hazard_risk?: number | null; facilities?: string[]; }
export interface RelocationAllocation { site_id: string; site_name: string; people: number; }
export interface RelocationPlan { village_id: string; village_name: string; population: number | null; best_site: any | null; ranked_sites: any[]; allocations: RelocationAllocation[]; fully_covered: boolean; reason: string | null; guided_flow?: string[]; scope?: { region?: string | null; district?: string | null }; }
...
export interface LiveHazard { id: string; type: "Earthquake" | "Fire Hotspot" | string; title?: string; lat?: number | null; lng?: number | null; magnitude?: number | null; confidence?: string | number; frp?: number | null; time?: number | string | null; severity: "CRITICAL" | "HIGH" | "MODERATE" | "LOW"; source: string; url?: string; detail?: string; }
