...
export default function Dashboard() {
  ...
  // Initial scope: restore only the State. The next effects restore District and City
  // after their parent options have actually loaded.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    api.regions.districts(state)
      .then(rows => {
        ...
        const names = rows.map(r => r.name).filter(Boolean).sort((a, b) => a.localeCompare(b));
        setDistricts(names);
        const saved = localStorage.getItem("rakshasetu_district") || "";
        if (saved && names.includes(saved)) setDistrict(saved);
      })
  ...
  // Flow step 3: District -> City / Town.
  useEffect(() => {
    let active = true;
    setCities([]);
    setCity("");
    ...
    setLocationLoading(true);
    api.regions.cities(state, district)
      .then(rows => {
        if (!active) return;
        const names = rows.map(r => r.name).filter(Boolean).sort((a, b) => a.localeCompare(b));
        setCities(names);
        const saved = localStorage.getItem("rakshasetu_city") || "";
        if (saved && names.includes(saved)) setCity(saved);
      })
      .catch(e => active && setError(e.message))
      .finally(() => active && setLocationLoading(false));
  ...
  // Operational data is loaded only after the complete location scope exists.
  useEffect(() => {
    let active = true;
    setError("");
    if (!state || !district) {
    ...
    Promise.all([
      ...
      api.live.hazards(state, district),
      api.live.news(state, district)
    ]).then(([v, s, live, n]) => {
      if (!active) return;
      setVillages(v);
      setSafeSites(s);
  ...
  async function selectVillage(v: Village) {
    setSelectedId(v.id);
    try { setDetail(await api.villages.get(v.id, { state, district })); }
    catch (e) { setError(e instanceof Error ? e.message : "Could not load settlement details"); }
  }

  function clearScope() {
    setState(""); setDistrict(""); setCity("");
    setDistricts([]); setCities([]); setVillages([]); setSafeSites([]);
    setLiveFeed({ updated_at: "", items: [], sources: [] }); setNews([]);
  ...
  return <div className="dashboard-page">
    ...
    <FilterBar states={states} state={state} setState={setState} districts={districts} district={district} setDistrict={setDistrict} cities={cities} city={city} setCity={setCity} loading={locationLoading} />
    ...
    {!state && <div className="panel" style={{ margin: "12px 0", padding: 16 }}>Start by selecting a State / Union Territory.</div>}
    {state && !district && <div className="panel" style={{ margin: "12px 0", padding: 16 }}>Now select a District in <strong>{state}</strong>. City / Town will load after the district is selected.</div>}
    {state && district && !city && <div className="panel" style={{ margin: "12px 0", padding: 16 }}>Now select a City / Town in <strong>{district}, {state}</strong>. Live dashboard data is scoped to the selected district.</div>}
    {loading && <div className="panel" style={{ margin: "12px 0", padding: 16 }}>Fetching live open data for <strong>{district}, {state}</strong>…</div>}

    <section className="command-grid"><div className="map-stage"><RiskMap villages={villages} safeSites={safeSites} liveHazards={mapHazards} selectedId={selectedId} onSelectVillage={selectVillage} region={state || "India"} district={district} /><div className="map-overlay-title"><strong>{district || state || "India"}</strong><span>{district ? `${villages.length} mapped locations · ${critical} critical · ${high} high` : "Choose State / UT → District → City / Town"}</span></div><div className="map-legend"><Legend color="var(--risk-critical)" label="Critical" /><Legend color="var(--risk-high)" label="High" /><Legend color="var(--risk-moderate)" label="Moderate" /><Legend color="var(--risk-low)" label="Low" /><Legend color="#20c7ff" label="Mapped safe site" /><Legend color="#ff1744" label="Live signal" /></div></div>
      <aside className="details-stage"><div className="panel-heading"><div><span className="eyebrow">SELECTED AREA</span><h2>{district || state || "India"}</h2></div><span className="status-dot">LIVE</span></div>{selectedId && detail ? <SelectedAreaPanel village={detail} onViewRelocation={() => navigate(`/relocation?village=${detail.id}`)} /> : district ? <RegionDetailsPanel region={state} district={district} villages={villages} safeSites={safeSites} recommendedSite={safeSites[0] || null} /> : <div className="region-empty panel"><span className="eyebrow">DATA SCOPE</span><h3>State / UT → District → City / Town</h3><p>Select the location in order. Data modules unlock after the district is selected.</p></div>}</aside>
    </section>
