...
export const api={
 ...
 live:{hazards:(region?:string)=>request<LiveHazardFeed>(`/api/live/hazards${region?`?region=${encodeURIComponent(region)}`:""}`),news:(region:string)=>request<any>(`/api/live/news?region=${encodeURIComponent(region)}`)},
