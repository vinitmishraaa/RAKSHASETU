...
export const api = {
  villages: {
    list: (params?: { district?: string; level?: string }) => { const q = new URLSearchParams(params as Record<string, string>).toString(); return request<Village[]>(`/api/villages${q ? `?${q}` : ""}`); },
  ...
  assistant: { ask: (question: string) => request<{ answer: string; grounded: boolean; used_llm: boolean }>("/api/assistant", { method: "POST", body: JSON.stringify({ question }) }) },
};
